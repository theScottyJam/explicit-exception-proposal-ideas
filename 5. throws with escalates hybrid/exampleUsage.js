...
function givePermissionToUsers(permission, userIds, { userIdDoingRequest }) throws UnauthorizedEx, NotFoundEx {
  ...
  // Get every user in the first place, so that if a NoFound exception occurs
  // we don't start making modifications to the users and stop halfway through
  const users = []
  for (const userId of userIds) {
    users.push(
      userManager.getUser(userId, { userIdDoingRequest }) escalates UnauthorizedEx
    )
  }
  ...
  for (const user of users) {
    if (!user.permissions.includes(permission)) {
      ...
      userManager.setProperty(user.id, 'permissions', newPermissions, { userIdDoingRequest }) escalates UnauthorizedEx, NotFoundEx
    }
  }
}
